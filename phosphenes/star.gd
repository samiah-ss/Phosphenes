...
func _pressed():
	...
	tween.tween_property(vl1, "modulate:a", 1.0, 0.5) # Incorporate a 0.5-second fade-in.
	tween.tween_interval(3.0) # Pause tweening effects for a 2-second readability window.
	tween.tween_property(vl1, "modulate:a", 0.0, 0.5) # Incorporate a 0.5-second fade-out.
	await tween.finished # Mark tweening as finished.
	vl1.visible = false # Undo dialogue visibility.
